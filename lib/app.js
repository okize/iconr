...
module.exports = (args, opts) => {
  // if sdout option set, suppress output noise
  if (opts.stdout) {
    opts.verbose = false; // eslint-disable-line no-param-reassign
    opts.analytics = false; // eslint-disable-line no-param-reassign
  }
  ...
  // if the output directory does not exist, create it and PNG directory
  ...
  return fsAsync
    ...
    .then((filteredFiles) => {
      ...
      filteredFiles.forEach((file) => {
        ...
        queue.push(fsAsync.readFileAsync(svgPath, 'utf8'));
    ...
    .then((svgData) => {
      ...
      svgData.forEach((svg) => queue.push(image.optimizeSvg(svg)));
    ...
    .then((data) => {
      ...
      return data.forEach((obj, i) => {
        ...
        return Object.assign(results[i], svgData);
      });
    })
    .then(() => {
      const queue = [];
    ...
    .then((unfilteredPngPaths) => {
      ...
      const pngPaths = unfilteredPngPaths.filter((pngPath) => pngPath !== undefined);
      ...
      if (pngPaths !== null) {
        ...
        pngPaths.forEach((pngPath, i) => {
          queue.push(fsAsync.readFileAsync(pngPath, null));
          ...
          if (!opts.nopng) {
            ...
            return Object.assign(results[i], { pngpath });
          }
          return null;
        });
      }
    ...
    .finally(() => {
      ...
      if (opts.analytics && showAnalytics) {
        appLog.appEnd = process.hrtime(appLog.appStart);
        analytics(appLog);
      }
    })
    .done();
