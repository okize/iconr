...
module.exports = {
  ...
  createCssRules: function(results, opts) {
    ...
    str = '';
    if (!opts.killcomment) {
      str += '/* generated by iconr */\n\n';
    }
    _.each(results, function(res, i) {
      var height, width;
      height = !isNaN(res.height) ? "height:" + res.height + "px;" : '';
      width = !isNaN(res.width) ? "width:" + res.width + "px;" : '';
      str += ("." + (cssClassnamePrefix + res.name) + "{") + ("" + height) + ("" + width) + ("background-image:url(" + res.svgdatauri + ");") + "}";
      if (!opts.nopngdata) {
        str += (".no-inlinesvg ." + (cssClassnamePrefix + res.name) + "{") + ("background-image:url(" + res.pngdatauri + ");") + "}";
      }
      if (!opts.nopng) {
